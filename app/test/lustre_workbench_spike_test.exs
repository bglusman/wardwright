defmodule WardwrightWeb.LustreWorkbenchSpikeTest do
  ...
  test "selected model simulation uses the real workbench policy path" do
    ...
    assert elem(simulation, 6) =~ "system/wardwright_policy_reminder: Include a small ASCII cow"
    assert elem(simulation, 8) == true

    assert Enum.any?(elem(simulation, 11), fn {_phase, label, detail, _severity} ->
