defmodule Wardwright.ToolMediation do
    ...
    Enum.reduce(tools, {[], []}, fn tool, {kept, matched_names} ->
      if rule_matches_tool?(rule, tool) do
        ...
        case patch_tool(tool, rule, action) do
          :hide -> {kept, append_present(matched_names, name)}
          {:replace, replacement} -> {kept ++ [replacement], append_present(matched_names, name)}
          {:keep, patched_tool} -> {kept ++ [patched_tool], append_present(matched_names, name)}
        end
      else
        {kept ++ [tool], matched_names}
      end
    end)
  end
  ...
  defp patch_tool(_tool, %{@tool_key => replacement}, @replace_action) when is_map(replacement) do
    {:replace, normalize_tool_schema(replacement)}
  end

  defp patch_tool(tool, _rule, _action), do: {:keep, tool}
  ...
  defp schema_hash(tool), do: "sha256:" <> Base.encode16(:crypto.hash(:sha256, JSON.encode!(tool)), case: :lower)
  ...
  defp append_present(list, nil), do: list
  defp append_present(list, value), do: list ++ [value]
