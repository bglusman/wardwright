...
async function loadExportedOmpRule() {
  const installedRulePath = process.env.WARDWRIGHT_OMP_RULE_PATH;
  if (installedRulePath) {
    return await readFile(installedRulePath, "utf8");
  }
...
async function validateAdapterConfig() {
  const configPath = process.env.WARDWRIGHT_OMP_CONFIG_PATH;
  if (!configPath) return;

  const config = JSON.parse(await readFile(configPath, "utf8"));
  const hasIdentity = config.paired === true && config.gateway_identity && typeof config.gateway_identity === "object";

  if (
    config.schema !== "wardwright.adapter_config.v0" ||
    config.adapter_id !== "wardwright-omp" ||
    config.target !== "omp" ||
    config.runtime !== "omp" ||
    !hasIdentity
  ) {
    throw new Error(`Wardwright OMP adapter config is not paired or does not match ${configPath}`);
  }
}
