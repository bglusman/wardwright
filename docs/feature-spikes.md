...
| Jido in-page authoring assistant | A narrow product assistant may be easier and safer than asking users to wire a general-purpose external agent into MCP. | LiveView chat panel backed by `jido_ai`, configured by environment, prompted with Wardwright's deterministic-artifact rules and the same authoring tool registry exposed to MCP/API clients. | The dependency footprint is large, chat can imply more authority than it has, and write-tool execution needs explicit confirmation semantics before production use. | A user can ask for a model change, receive a correct tool plan and review checklist, then validate/simulate the result faster than with a generic agent. |
...
| Wardwright-hosted server tools | Models may need governed read-only facts such as receipt summaries, recent policy state, or derived session predicates without gaining broad local execution authority. | `0.0.11` adds a minimal server-tool registry/execution framework for non-streaming Chat Completions. It includes one explicit read-only built-in, `wardwright_policy_cache_status`, plus trusted Dune function tools and trusted BEAM module tools loaded from `.ex/.exs`, `.erl`, or `.beam` paths. Next spikes should register more bounded tools behind the same framework and expose the same model through MCP/protected API or a future Responses-style surface; record execution location, visibility level, arguments hash, result metadata, denial, timing, and receipt evidence. | Hidden broad Chat Completions tool injection would bypass framework-native tool state, approvals, tracing, and graph checkpoints. Dune and BEAM module extensions are trusted operator code, not hostile-code sandboxes; side-effecting tools need stronger authorization and approval semantics. | A model or agent can request a bounded Wardwright fact or trusted local extension, and the receipt/UI can explain who executed it, what was observed, what was redacted, and why policy allowed or denied it. |
...
- Full arbitrary receipt queries inside policy code: use deterministic declared
  working sets first.
- Hidden broad Wardwright-owned tool execution in Chat Completions: keep the
  `0.0.11` tools explicit, trusted-local, and receipted, then use MCP, protected
  API, or a future Responses-style surface before adding richer extension tools
  so framework-native tool state and Wardwright receipts do not disagree.
