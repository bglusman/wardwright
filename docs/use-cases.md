...
| Counterfactual replay debugger | Tests whether a new policy would have prevented an agent failure. | Full session transcript, event cursor, fork policy overlay, continuation result, comparison verdict. | Record a read-before-edit failure, replay to the unsafe edit, fork with a read-before-edit policy, continue the fork, and assert the fixed run passes. |
